...
export default function EditCampaignPage() {
  ...
  async function saveCampaign(newStatus: 'draft' | 'published') {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20" style={{ color: 'var(--admin-text-muted)' }}>
        <span className="text-sm">טוען קמפיין...</span>
      </div>
    )
  }
        ...
        {status === 'published' && slug && (
          <>
            <button
              ...
              style={{ background: 'var(--admin-bg-elevated)', border: '1px solid var(--admin-border)', color: copied ? 'var(--admin-success)' : 'var(--admin-text-primary)' }}
            >
