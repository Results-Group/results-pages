  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#0c0e10', border: '1px solid var(--admin-border)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { onClose(); reset() }} className="p-1 rounded-lg transition-colors" style={{ color: 'var(--admin-text-secondary)' }} aria-label="סגור">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <label className="block text-[11px] font-bold mb-1.5" style={{ color: 'var(--admin-text-secondary)' }}>סוג מוקאפ (חל על כל השקפים שייווצרו)</label>
          ...
          {SMART_TYPES.map(t => (
            <button
              ...
              style={mockupType === t.value
                ...
                : { background: 'var(--admin-hover-bg)', border: '1px solid var(--admin-border)', color: 'var(--admin-text-secondary)' }}
            >
              {t.icon} {t.label}
            </button>
          ))}
        ...
        <div
          ...
          style={{
            border: `1.5px dashed ${dragOver ? 'rgba(64,225,211,0.6)' : 'var(--admin-border-input)'}`,
            background: dragOver ? 'rgba(64,225,211,0.06)' : 'var(--admin-hover-bg)',
          }}
        >
          <UploadCloud className="w-7 h-7" style={{ color: dragOver ? '#40e1d3' : 'var(--admin-text-muted)' }} />
          <p className="text-xs font-semibold text-white">גרור קבצים לכאן או לחץ לבחירה</p>
          <p className="text-[11px]" style={{ color: 'var(--admin-text-muted)' }}>ניתן לבחור הרבה קבצים יחד</p>
          <input
            ref={inputRef}
            type="file"
            accept="image/*"
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button
            ...
            style={{ color: 'var(--admin-text-secondary)', background: 'var(--admin-hover-bg)' }}
          >
            ביטול
          </button>
          <button
